    ...
    if run:
        ...
        with st.spinner("Analysing..."):
            ...
            try:
                ...
                # Auto-add result summary to chat
...
with col_chat:
    ...
    if send and user_input.strip():
        ...
        if st.session_state.last_result:
            r = st.session_state.last_result
            result_context = f"""
The patient's latest assessment result:
- Risk Label: {r['risk_label']}
- Risk Probability: {r['risk_probability']*100:.1f}%
- Glucose: {r.get('glucose', 'N/A')} mg/dL
- BMI: {r.get('bmi', 'N/A')}
- Age: {r.get('age', 'N/A')}
- Advice given: {r['advice']}
"""

        system_prompt = f"""You are a friendly, empathetic medical AI assistant inside a diabetes risk prediction app called MediPredict. 
Your job is to help patients understand their results, explain medical terms in simple language, and provide general health guidance.
...
- Format nicely but avoid markdown headers"""
        ...
        try:
            ...
            data = response.json()
            bot_reply = data["content"][0]["text"]
        except Exception as e:
            bot_reply = "Sorry, I'm having trouble connecting right now. Please try again in a moment."
